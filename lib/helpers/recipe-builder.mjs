...
import defaultRecipe from '@juquinha/config/default-recipe.mjs'
import defaultApiRecipe from '@juquinha/config/default-api-recipe.mjs'
import defaultFunctionRecipe from '@juquinha/config/default-function-recipe.mjs'
...
export default class RecipeBuilder {
  ...
  usesDynamoTable (tableName) {
    ...
    const fullTableName = `\${self:custom.appPrefix}-\${self:provider.stage}-${tableName}-table`
    const environmentTableName = `${tableName.toUpperCase().replace(/-/g, '_')}_TABLE_NAME`
    this.setEnvironment(environmentTableName, fullTableName)
    const customTableName = kebabToCamel(tableName)
    this.setCustom(customTableName, fullTableName)
    return this
  }

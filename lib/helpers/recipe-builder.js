...
const { kebabToCamel } = require('lib/common/helpers/string')
...
module.exports = class RecipeBuilder {
  ...
  addFunction (functionName, functionObject) {
    ...
    if (!this.__recipe.provider.iam.role.statements.find(statement => statement.Effect === 'Allow' && statement.Action.startWith('logs:'))) {
  ...
  usesDynamoTable (tableName) {
    ...
    this.setEnvironment(`${tableName.toUpperCase()}_TABLE_NAME`, '${self:custom.rolesTableName}')

        ...
        slot(name='create.title' :context='context')
          v-col.flex-grow-1
            slot(name='title' :context='context')
              span.white--text.text--break-on-word {{ $i18n('create') }} {{ context.entityName.toLowerCase() }}
          v-col.flex-grow-0.text-no-wrap
            ...
            v-tooltip(bottom)
              template(v-slot:activator='{ on: closeOn, attrs: closeAttrs }')
                v-btn.white--text(icon @click='dialog = false' v-bind='closeAttrs' v-on='closeOn'): v-icon mdi-close
              span {{ $i18n('close') }}
    v-card-text
      Form(
        :key='itemKey'
